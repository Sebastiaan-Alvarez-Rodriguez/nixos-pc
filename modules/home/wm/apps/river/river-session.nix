# All this stuff to get river running decently.
{ config, lib, pkgs, ...}: let
  ...
  mkBindings = bindings: command: mode: lib.concatStrings (lib.mapAttrsToList (bind: action: "riverctl ${command} ${mode} ${bind} ${action}\n") bindings);
...
in with lib; {
  options.programs.river = {
    ...
    systemdIntegration = mkOption {
      ...
      description = "Whether to enable `river-session.target` on river startup. This links to `graphical-session.target`.";
    };
  ...
  config = lib.mkIf cfg.enable {
    systemd.user.targets.river-session = lib.mkIf cfg.systemdIntegration {
      Unit = {
