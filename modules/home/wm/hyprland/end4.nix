{ config, inputs, lib, pkgs, ... }: let
  cfg = config.my.home.wm.hyprland.end4-illogical;
in {
  imports = [ inputs.end4-illogical.homeManagerModules.default ];
  options.my.home.wm.hyprland.end4-illogical = with lib; {
    enable = mkEnableOption "Use end4 theme for hyprland";

    binds = {
      modkey = mkOption {
        type = types.str;
        default = "SUPER"; # This is the 'windows' key on most keyboards.
        description = "Modkey to use for issuing commands to hyprland.end4";
      };
    ...
    extra-config = mkOption {
      type = types.attrs;
      default = {};
      description = "Extra lines to append to config of end4";
    };
  };

  config = lib.mkIf cfg.enable {
    # NOTE:
    # Ensure you also have enabled config.my.system.hyprland-end4.enable.
    ...
    programs.illogical-impulse = {
      ...
      # hyprland.plugins = [ # seb NOTE: DO NOT USE extra plugins using pkgs - does not use correct nixpkgs versio
      ...
      sections = {
        "Window Management" = {
          "Main".bindd = [ # toggle between fullscreen or not
            ...
            # "$mainMod Shift, F, $d toggle pin on focused window, exec, hyde-shell windowpin" # seb TODO
          ];
        ...
        "Launcher" = {
          "Apps".bindd = [
            ...
            # "Control SHIFT, Escape, $d system monitor, exec, hyde-shell sysmonlaunch" # note: uses "system.monitor" in upstream config # seb TODO
          ];
          "Rofi menus".bindd = [
            # seb TODO: replace command below with end4 commands
