...
in {
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      { assertion = builtins.elem "dell-smm-hwmon" config.boot.initrd.kernelModules; message = "Must have `dell-smm-hwmon` in `boot.initrd.kernelModules`!"; }
      { assertion = builtins.elem "dell-smm-hwmon" config.boot.kernelModules; message = "Must have `dell-smm-hwmon` in `boot.kernelModules`!"; }
    ];
