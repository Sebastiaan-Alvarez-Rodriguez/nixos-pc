# Taken from https://gist.github.com/Infinisil/c68a2f385c6d7c52c324d529b7f1d07c
{ lib, ... }: let
  inherit (lib) bitAnd concatMapStringsSep concatStringsSep elemAt foldr genList id mod range toInt warn zipListsWith;
in rec {
  ...
  parseSubnet4 = str: let
    ...
    try = if baseIp == givenIp then id else warn (concatStringsSep " " [
    ...
    nth = n: let
      ...
      try = if check result then id else warn (concatStringsSep " " [
  ...
  nthInRange4 = { from, to }: n: let
    carry = lhs: { carry, acc }: let
      totVal = lhs + carry;
    in {
      carry = totVal / 256;
    ...
    checkInRange = if (to - from) < n then warn "nthInRange4: '${n}'-th address outside of range (${prettyIp4 from}, ${prettyIp4 to})" else id;
  in
    checkInRange carried.acc;

  # Convert an IPv4 range into a list of all its constituent addresses
  rangeIp4 = { from, to } @ arg: let
    numAddresses = builtins.foldl' (acc: rhs: acc * 256 + rhs) 0 (zipListsWith (lhs: rhs: lhs - rhs) to from);
  in
    map (nthInRange4 arg) (range 0 numAddresses);

  isValidIp4 = ip: (builtins.all (n: n >= 0 && n < 256) ip) && (builtins.length ip == 4);
}
